# AGENTS.md — Invictus CRM (Verena Self-Launch)

Every Codex session working in this repo reads this file. It is the shared contract
...
| D9 | Foundation and each feature agent run as a **separate Codex Desktop session**, one per git worktree. |
...
`.Codex/settings.json`. Auto mode's classifier approves routine work — including merges and
...
from `.Codex/settings.json` or `.Codex/settings.local.json` — set there, the session
